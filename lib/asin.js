...
async function resolverLink(entrada) {
  ...
  if (asinDireto) {
    return {
      asin: asinDireto,
      linkParaExibir: `https://www.amazon.com.br/dp/${asinDireto}?tag=${TAG_AFILIADO}`
    };
  }
  ...
  try {
    ...
    return {
      asin: asinResolvido,
      linkParaExibir: `https://www.amazon.com.br/dp/${asinResolvido}?tag=${TAG_AFILIADO}`
    };
  } catch (err) {
    return { asin: null, linkParaExibir: null };
  }
...
module.exports = { resolverLink, extrairASIN, expandirLinkAmazon };
